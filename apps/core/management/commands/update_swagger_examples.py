...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--endpoint',
            type=str,
            help='Update examples for specific endpoint only'
        )
        parser.add_argument(
            '--clear',
            action='store_true',
            help='Clear all captured examples'
        )
    ...
    def handle(self, *args, **options):
        ...
        if not examples:
            self.stdout.write(
                self.style.WARNING('No captured examples found. Make some API calls first.')
            )
            return
    ...
    def update_single_endpoint(self, endpoint_name, endpoint_examples):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Updated {endpoint_name} examples')
        )
    ...
    def update_all_endpoints(self, examples):
        ...
        for endpoint_name, endpoint_examples in examples.items():
            ...
            for method, method_examples in endpoint_examples.items():
                for status_code, example in method_examples.items():
                    total_examples += 1
                    timestamp = example.get('description', '').replace('Captured response from ', '')
                    self.stdout.write(
                        f'  ✅ {method} {status_code} - {timestamp[:19]}'
                    )
