...
class PaymentProvider(TimeStampedModel):
    ...
    is_active = models.BooleanField(default=True, help_text="Whether provider is active")
    ...
    test_mode = models.BooleanField(
        default=True,
        help_text="Whether to use test mode"
    )
...
class PaymentMethod(TimeStampedModel):
    ...
    provider_method_id = models.CharField(
        max_length=200,
        help_text="Payment method ID from provider"
    )
...
class Cart(TimeStampedModel):
    ...
    notes = models.TextField(
        blank=True,
        help_text="Customer notes for the cart"
    )
    metadata = models.JSONField(
        default=dict,
        help_text="Additional cart metadata"
    )
    ...
    expires_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text="When the cart expires"
    )
...
class CartItem(TimeStampedModel):
    ...
    custom_attributes = models.JSONField(
        default=dict,
        help_text="Custom attributes for the item"
    )
    notes = models.TextField(
        blank=True,
