        ...
        try:
            ...
            try:
                provider = PaymentProvider.objects.get(
                    provider_type=PaymentProvider.ProviderType.CHAPA, is_active=True
                )
            ...
            if webhook_status == "success" and result.get("verified"):
                ...
                transaction.save()
                logger.info(
                    f"Transaction {tx_ref} marked as {webhook_status} via webhook"
                )
                ...
                transaction.save()
                logger.info(
                    f"Updated webhook data for transaction {tx_ref}, status: {webhook_status}"
                )
    ...
    request_body=openapi.Schema(
        ...
        properties={
            ...
            "status": openapi.Schema(
                type=openapi.TYPE_STRING, description="Payment status"
            ),
            "amount": openapi.Schema(
                type=openapi.TYPE_NUMBER, description="Payment amount"
            ),
            "currency": openapi.Schema(
                type=openapi.TYPE_STRING, description="Payment currency"
            ),
        },
    ),
    responses={
        ...
        404: openapi.Response(
            "Provider not found",
            examples={"application/json": {"error": "Chapa provider not found"}},
        ),
    },
)
@api_view(["POST"])
...
def chapa_webhook_api(request):
    ...
    try:
        ...
        try:
            ...
            return Response(
                {"error": "Chapa provider not found"}, status=status.HTTP_404_NOT_FOUND
            )
