    ...
    def update(self, instance, validated_data):
        ...
        for index, image in enumerate(images):
            ProductImage.objects.create(
                ...
                image=image,
                sort_order=index,
                is_primary=(index == 0 and not instance.images.filter(is_primary=True).exists())
            )
...
class EnhancedProductListSerializer(ProductListSerializer):
    ...
    variants = ProductVariantListSerializer(source="available_variants", many=True, read_only=True)
    ...
    def to_representation(self, instance):
        ...
        data["total_stock"] = sum(
            variant.stock_quantity for variant in instance.variants.filter(is_active=True)
        ) if instance.has_variants else instance.stock_quantity
...
class EnhancedProductDetailSerializer(ProductDetailSerializer):
    ...
    variants = ProductVariantListSerializer(source="available_variants", many=True, read_only=True)
    all_variants = ProductVariantListSerializer(source="variants", many=True, read_only=True)
    variants_count = serializers.IntegerField(read_only=True)
    total_stock = serializers.IntegerField(read_only=True)
    ...
    def to_representation(self, instance):
        ...
        data["total_stock"] = sum(
            variant.stock_quantity for variant in instance.variants.all()
        ) if instance.has_variants else instance.stock_quantity
...
class ProductBulkOperationSerializer(serializers.Serializer):
    ...
    price_adjustment_type = serializers.ChoiceField(
        choices=[("set", "Set Price"), ("increase", "Increase"), ("decrease", "Decrease")],
        required=False,
        default="set",
    ...
    stock_quantity = serializers.IntegerField(
        required=False,
        help_text="Stock quantity to set (for update_stock action)"
    )
    
    category_id = serializers.IntegerField(
        required=False,
        help_text="Category ID to assign (for assign_category action)"
    )
    ...
    def validate(self, data):
        ...
        if action == "update_price" and not data.get("price_adjustment"):
            raise serializers.ValidationError(
                {"price_adjustment": "Price adjustment is required for update_price action"}
            )
...
class ProductImportSerializer(serializers.Serializer):
    ...
    file = serializers.FileField(
        help_text="CSV or Excel file containing product data"
    )
    
    update_existing = serializers.BooleanField(
        default=False,
        help_text="Whether to update existing products with the same SKU"
    )
...
class ProductExportSerializer(serializers.Serializer):
    ...
    date_from = serializers.DateTimeField(
        required=False,
        help_text="Filter products created from this date"
    )
    
    date_to = serializers.DateTimeField(
        required=False,
        help_text="Filter products created until this date"
    )
