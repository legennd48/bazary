...
class ProductManagementViewSet(viewsets.GenericViewSet):
    ...
    def bulk_operations(self, request):
        ...
        try:
                ...
                if action_type == "activate":
                    ...
                    for product in products:
                        if adjustment_type == "set":
                            ...
                            product.price = max(
                                Decimal("0.01"), product.price - price_adjustment
                            )
                        product.save()
                        affected_count += 1
            ...
            return Response(
                {"success": False, "message": f"Bulk operation failed: {str(e)}"},
    ...
    def upload_images(self, request, pk=None):
        ...
        for index, image in enumerate(images):
            ProductImage.objects.create(
                ...
                sort_order=index,
                is_primary=(
                    index == 0 and not product.images.filter(is_primary=True).exists()
                ),
            )
            uploaded_count += 1
    ...
    def import_products(self, request):
        ...
        try:
            ...
            if file.name.endswith(".csv"):
                ...
                return Response(
                    {"success": False, "message": "Excel import not implemented yet"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
                ...
                for row_num, row in enumerate(reader, start=2):
                    try:
                        ...
                        if existing_product and not update_existing:
                            errors.append(
                                f"Row {row_num}: Product with SKU {sku} already exists"
                            )
                            continue
    ...
    def export_products(self, request):
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="products_export_{timestamp}.csv"'
        )
        ...
        for product in queryset:
            ...
            if include_variants:
                ...
                row.extend(
                    [variants.count(), ", ".join(variant.sku for variant in variants)]
                )
